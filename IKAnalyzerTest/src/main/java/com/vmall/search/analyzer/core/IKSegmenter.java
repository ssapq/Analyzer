...
public final class IKSegmenter {
	 ...
	 */
	public IKSegmenter(Reader input , boolean useSmart, boolean useFragment){
		this.input = input;
		this.cfg = DefaultConfig.getInstance();
		this.cfg.setUseSmart(useSmart);
		this.cfg.setUseFragment(useFragment);
		this.init();
	}
	 ...
	 * @throws IOException
	 */
	public synchronized Lexeme next(Dictionary dictionary)throws IOException{
		Lexeme l = null;
		while((l = context.getNextLexeme(dictionary)) == null ){
			/*
			...
			if(available <= 0){
				...
				do{
        			...
        			for(ISegmenter segmenter : segmenters){
        				segmenter.analyze(context,dictionary);
        			}
        			//字符缓冲区接近读完，需要读入新的字符
