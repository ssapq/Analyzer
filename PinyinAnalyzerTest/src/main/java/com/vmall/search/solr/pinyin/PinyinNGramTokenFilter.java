 ...
 * token.
 * <p>
 * <a name="match_version"></a>As of Lucene 4.4, this filter handles correctly
...
//	private int tofStart;
//	private int tofEnd; // only used if the length changed before this filter
	...
	private boolean hasCurOut = false; // 当前输入是否已输出back
		...
		while (true) {
			if (curTermBuffer == null) {
				if (!input.incrementToken()) {
					...
					curGramSize = minGram;
					// tofStart = offsetAtt.startOffset();
					// tofStart =side == Side.FRONT ? 0 : curTermLength -
					// curGramSize;
					// tofEnd = offsetAtt.endOffset();
					// tofEnd = tofStart + curGramSize;
			...
			if (curGramSize <= maxGram) { // if we have hit the end of our
				...
				if (curGramSize <= curCodePointCount) { // if the remaining
					...
					// charUtils.offsetByCodePoints(curTermBuffer, 0,
					// curTermLength, 0, curGramSize);
					// termAtt.copyBuffer(curTermBuffer, tofStart, curGramSize);

					if (!hasCurOut) {
						hasCurOut = true;
						//back 
						int start =  curTermLength - curGramSize;
